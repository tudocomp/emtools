...
/**
 * \brief External Sorter: use stream package objects to keep a sorted container.
 *
...
public:
    ...
    typedef typename runs_creator_type::element_iterator element_iterator;
...
public:
    ...
    long number_of_runs() {
        return (long)m_runs_creator.result()->runs.size();
    }

    long min_merge_budget() {
        long nruns = (long)m_runs_creator.result()->runs.size();
        long ndisks = (long)config::get_instance()->disks_number();
        return (nruns + 2 * ndisks + 1) * block_size;
    }
    ...
    //! \brief Standard stream method (preincrement operator)
    sorter& operator ++ ()
    {
        assert( m_state == STATE_OUTPUT );
        ++m_runs_merger;
